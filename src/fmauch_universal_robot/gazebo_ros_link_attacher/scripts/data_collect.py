...
def set_move_vertical_capture(ee_pose,   x_bolt, y_bolt, z_bolt):
    ...
    if x_delta==0 or x_delta==1 or x_delta==-1:
        x_delta=2
    x_delta_random = float(x_delta)/float(1000.0)

    y_delta = random.randint(-50, 50)
    if y_delta==0 or y_delta==1 or y_delta==-1:
        y_delta=2
    y_delta_random = float(y_delta)/float(1000.0)
...
if __name__ == '__main__':
    ...
    bolt_type="cross_bolt"
    predicate_type='not_aligned_with_obstacles'
