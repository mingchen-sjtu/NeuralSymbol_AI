...
class YOLO(LabelWriterBase):
    ...
    def outputScene(self, robot_list, raw_image, camera_info):
        ...
        raw_image_file = self._output_location + 'img/' + id_string + '.png'
        cv2.imwrite(filename=raw_image_file, img=raw_image)
        # Add the label file
        label_file = self._output_location + 'txt/' + id_string + '.txt'
        with open(label_file, 'w') as file:
            ...
            for robot in robot_list:
                ...
                if (0<center_x<1.0 and 0<center_y<1.0):
                    if not(0.35<center_x<0.51 and 0.56<center_y<0.88):
