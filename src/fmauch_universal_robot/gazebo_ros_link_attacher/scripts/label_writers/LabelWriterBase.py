...
class LabelWriterBase(object):
    ...
    def __init__(self, name):
        ...
        self._tf_timeout = ParameterLookup.lookupWithDefault(
            parameter=timeout_param, default=0)
    ...
    def _projectPointIntoFrame(self, point, source_frame_id, target_frame_id):
        ...
        point_source_stamped.header.stamp = rospy.Time()
        ...
        print(point_source_stamped)
        print(target_frame_id)
        try:
            self._tf_listener.waitForTransform(target_frame_id, source_frame_id,rospy.Time(), timeout)
            point_target_stamped = self._tf_listener.transformPoint(target_frame_id, point_source_stamped)
