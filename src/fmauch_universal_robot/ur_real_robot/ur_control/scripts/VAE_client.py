...
class VAE_SendImg():
    ...
    def finish_VAE_detect(self, result):
        ...
        for bolt_c in circlescenter:
            dis=math.sqrt(pow(bolt_c[0] - 301 ,2)+pow(bolt_c[1] - 215 ,2))
            if dis<min_dis:
                min_dis=dis
                aim_bolt=i
            i+=1   
        ...
        if result == 'cross_bolt':
            result='cross_bolt'
        elif result == 'star_bolt':
            result='star_bolt'
        elif result == 'hexagon_bolt':
            result='hexagon_bolt'
        elif result == 'm_bolt':
