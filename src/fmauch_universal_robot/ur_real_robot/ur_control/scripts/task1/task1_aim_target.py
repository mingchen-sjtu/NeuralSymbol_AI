...
from task1_base import TrueBase
            ...
            if detect_ret:
                ...
                if (s==0):
                    ...
                    if  np_collected==False and (not temp_diff_list==[]):
                        ...
                        cspose_x = coarse_pose_list[temp_diff_list.index(min(temp_diff_list))].position.x
                        cspose_y = coarse_pose_list[temp_diff_list.index(min(temp_diff_list))].position.y
                        coarse_pose.position.z = 0.65


                        # q = tf.transformations.quaternion_from_euler(-math.pi, 0, -0.5*math.pi)
                        if (cspose_x < 0.265):
                            coarse_pose.position.x = cspose_x - 0.065
                            coarse_pose.position.y = cspose_y
                            q = tf.transformations.quaternion_from_euler(-math.pi, 0, -0.5*math.pi)
                        elif (cspose_x > 0.265 and cspose_x < 1.065):
                            coarse_pose.position.x = cspose_x
                            coarse_pose.position.y = cspose_y - 0.065                            
