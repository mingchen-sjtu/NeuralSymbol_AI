...
# import numpy as np
from sim_aim_target import SimAimTarget
from sim_move import SimMove
from sim_clear_obstacle import SimClearObstacle
from sim_recognition import SimRecognition
from sim_change import SimChange
from sim_insert import SimInsert
from sim_disassemble import SimDisassemble
from sim_action import PrimAction
...
class TSTPlanner:
    def __init__(self, camera_name, rgb_topic, depth_topic, camera_info_topic):
        ...
        self.stage={'have_coarse_pose':False, 'above_bolt':False,'target_aim':False, 'target_clear':False,'above_center_c':False,'target_match':False,'cramped':False,'disassembled':False}

        #初始化stage
        self.move_prim=SimMove(self.group)
        self.aim_target_prim = SimAimTarget(self.group)
        self.clear_obstacle_prim=SimClearObstacle(self.group)
        self.recognition_prim = SimRecognition(self.group)
        self.change_prim=SimChange(self.group)
        self.insert_prim=SimInsert(self.group)
        self.disassemble_prim=SimDisassemble(self.group)
    ...
    def auto_plan(self,original_stage):
        ...
        prim_list=(move,mate,push,recognition,change,insert,disassemble)
                    ...
                    if self.action in prim_dict.keys():
                        ...
                        if pre_is_ok==True:
                            ...
                            if self.action=='change':
                                self.ret_dict = prim.action(infos, self.ret_dict,filter,self.bolt_detector,self.sleeve_type,self.aim_bolt_type)
                            ...
                            if self.action=='disassemble':
                                filter.release()
                                filter=Kalman(20)
                                self.stage={'have_coarse_pose':True, 'above_bolt':False,'target_aim':False, 'target_clear':False,'above_center_c':False,'target_match':False,'cramped':False,'disassembled':False}
    ...
    def callback(self, rgb_msg, depth_msg):
        try:
            if not self.camera_model_ready:
                ...
                return
            img = self.bridge.imgmsg_to_cv2(rgb_msg, "rgb8")
...
if __name__ == '__main__':
    ...
    try:
        rospy.init_node('simplanner-moveit', anonymous=True)
        ...
        # pose_target.orientation.x = 0.98676
        # pose_target.orientation.y = -0.16203
        # pose_target.orientation.z = 0.00521
        # pose_target.orientation.w = -0.004466
