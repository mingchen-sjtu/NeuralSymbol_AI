...
from architectures.SEAttention import SEAttention
from architectures.CBAM import CBAMAttention
...
class ResBlock_Downsample(nn.Module):
    def __init__(self, in_channels, out_channels, depth=2, kernel_size=3, dropout=0):
        ...
        self.SEAttention_layer = SEAttention(channel=out_channels, reduction=8)
        self.CBAMAttention_layer = CBAMAttention(channel=out_channels, reduction=8, kernel_size=kernel_size)

    def forward(self, feat):
        return self.res_layer(feat) + self.skip_layer(feat)

    # def forward(self, feat):
    #     return self.SEAttention_layer(self.res_layer(feat)) + self.skip_layer(feat)

    # def forward(self, feat):
    #     return self.CBAMAttention_layer(self.res_layer(feat)) + self.skip_layer(feat)

    # def forward(self, feat):
    #     return self.SEAttention_layer(self.res_layer(feat)) + self.skip_layer(feat) + self.res_layer(feat)
...
class ConvToLin(nn.Module):
    ...
    def forward(self, feat):
        ...
        feat = feat.contiguous().view((batch, channels * width * height))
        return feat
...
class VAE_TinyResNet_dual_branch(nn.Module):
    """
    ...
    def __init__(self, opt):
        ...
        self.rgb_channels = opt['rgb_channels']
        self.depth_channels = opt['depth_channels']
        ...
        self.enc_out_channel_list = [64, 128]

        #--- Encoder network
        self.enc_conv = nn.Sequential()

        self.enc_conv.add_module('enc_conv0', nn.Conv2d(
                self.rgb_channels, self.enc_out_channel_list[0], self.enc_kernel_list[0],
        ...
        # encoder depth/mask branch
        self.enc_branch_2 = copy.deepcopy(self.enc_conv)
        self.enc_branch_2[0] = nn.Conv2d(
                self.depth_channels, self.enc_out_channel_list[0], self.enc_kernel_list[0],
                stride=2, padding=int((self.enc_kernel_list[0]-1)/2))
        # print('branch 2 first model: ', self.enc_branch_2[0])
        ...
        self.enc_branch_2.add_module('enc_fcscale', FCBlock(
                self.fc_dim, self.fc_dim, 1, self.dropout))
        self.enc_branch_2.add_module('P_enc_afcs',TempPrintShape('Output of enc_fcscale'))

        self.enc_mean = nn.Linear(self.fc_dim * 2, self.latent_dim)
        self.enc_logvar = nn.Linear(self.fc_dim * 2, self.latent_dim)
...
#        scales, dims = self.get_decoders_shape()
        ...
        self.dec_fc.add_module('dec_lin0', nn.Linear(
        ...
        # Output mean
        self.dec_mean_rgb = nn.Sequential()
        self.dec_mean_rgb.add_module('dec_mean_rgb', Upsample(
                dims[-1], self.rgb_channels, self.kernel_size))
        if opt['out_activation'] == 'sigmoid':
            self.dec_mean_rgb.add_module('dec_meanact', nn.Sigmoid())

        # Output mean
        self.dec_mean_depth = nn.Sequential()
        self.dec_mean_depth.add_module('dec_mean_depth', Upsample(
                dims[-1], self.depth_channels, self.kernel_size))
        if opt['out_activation'] == 'sigmoid':
            self.dec_mean_depth.add_module('dec_meanact', nn.Sigmoid())
    ...
    def encoder(self, x):
        ...
        x1 = x[:, :self.rgb_channels, :, :]
        x2 = x[:, self.rgb_channels:, :, :]

        rgb = self.enc_conv(x1) # (batch_size, lin_before_latent_dim)
        depth = self.enc_branch_2(x2)     
    ...
    def decoder_mean(self, z):
        ...
        """
        z_rgb = self.dec_fc(z)
        x_rgb = self.dec_conv(z_rgb)
        mean_rgb = self.dec_mean_rgb(x_rgb)

        z_depth = self.dec_fc(z)
        x_depth = self.dec_conv(z_depth)
        mean_depth = self.dec_mean_depth(x_depth)

        mean = torch.cat((mean_rgb, mean_depth), dim=1)
